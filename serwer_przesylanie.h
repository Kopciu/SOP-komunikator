...
#include <sys/msg.h>
#include <string.h>
#include "serwer_tworzenie.h"
#define idleTimes 5

//funckja-petla odpowiedzialna za odbieranie wiadomosci od klientow
void odbieranieWiadomosci(int ppid);
//wysyla dana wiadomosc do wszystkich klientow na danym kanale
void wyslijDoWszystkichNaKanale(message msg);
//funckja-petla odpowiedzialna za odbieranie komunikatow od klientow
void odbieranieKomunikatowKlienta(int ppid);
//funckja-petla odpowiedzialna za odbieranie komunikatow od innych serwerow
void odbieranieKomunikatowSerwera(int ppid);
//przetwarza dana wiadomosc
void odbierz(message msg);
//wysyla strukture pingClient o danym msgCode do klienta o danym id
...
while(ppid==getppid()){
	...
	//semafor
	opuscD();
	if(getKolejkeUseraD(m.sender)==-1 && m.msgCode!=CODE_SENDER_SERVER){
		podniesD();
		if(config[0]==1)
